...
export async function GET(req: NextRequest) {
  try {
    ...
    const filters = parseFilters(url, emailTemplateFilterSchema, [
      "category",
      "archived",
    ]);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const input = await parseBody(req, createEmailTemplateSchema);

    const service = new EmailTemplateService(session.organizationId);
    const template = await service.create({ userId: session.userId }, input);

    return success(template, { message: "Email template created.", status: 201 });
  } catch (error) {
    return failure(error);
  }
}
