...
import { buildExport, type ExportFormat } from "@/server/services/opportunity-export";
...
export async function GET(req: NextRequest) {
  try {
    const session = await requireApiContext("opportunities.export");
    ...
    const filename = `opportunities-${new Date().toISOString().slice(0, 10)}.${file.extension}`;
